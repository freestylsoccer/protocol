...
makeSuite('LendingPoolConfigurator', (testEnv: TestEnv) => {
  ...
  it('Update project borrower in the ETH reserve ', async () => {
    const { configurator, pool, helpersContract, weth } = testEnv;

    await configurator.updateProjectBorrower(weth.address, "0x2c5CeF061409B80e48b6e9cCc636Ebe58023d1A9");
  ...
  it('Update rates in the ETH reserve ', async () => {
    ...
    const {
      ...
      variableBorrowRate,
      stableBorrowRate
    } = await helpersContract.getReserveData(weth.address);
  ...
  it('Check the onlyAaveAdmin on updateProjectBorrower ', async () => {
    ...
    await expect(
      configurator.connect(users[2].signer).updateProjectBorrower(weth.address, "0x2c5CeF061409B80e48b6e9cCc636Ebe58023d1A9"),
      CALLER_NOT_POOL_ADMIN
    ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
  });
