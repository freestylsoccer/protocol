...
makeSuite('Deposit, Borrow, Withdraw, Repay ', (testEnv: TestEnv) => {
  ...
  it('User 1 deposits 1000 dai and project borrower borrow 1000 dai', async () => {
    ...
    await configurator.updateProjectBorrower(dai.address, users[1].address);

    // user 0 deposits 1000 DAI
    await dai.connect(users[0].signer).approve(pool.address, APPROVAL_AMOUNT_LENDING_POOL);

    await pool.connect(users[0].signer).deposit(dai.address, "0x3635c9adc5dea00000", users[0].address, '0');

    await pool.connect(users[1].signer).borrow(dai.address, "0x3635c9adc5dea00000", users[1].address);
  ...
  it('User try to borrow 1000 dai', async () => {
    ...
    await expect(
      pool.connect(users[0].signer).borrow(dai.address, "0x3635c9adc5dea00000", users[0].address),
      "Only project borrower"
    ).to.be.revertedWith("Only project borrower");
  });

  it('User try to Withdraw dai ', async () => {
    const { users, pool, dai, aDai, configurator, helpersContract } = testEnv;
    ...
    await configurator.updateProjectBorrower(dai.address, users[2].address);

    await pool.connect(users[2].signer).borrow(dai.address, "0x3635c9adc5dea00000", users[2].address);

    const {
      availableLiquidity
    } = await helpersContract.getReserveData(dai.address);

    console.log(availableLiquidity);

    await pool.connect(users[0].signer).withdraw(dai.address, amountDAItoDeposit, users[0].address);

    console.log(await dai.balanceOf(users[0].address));
    console.log(await dai.balanceOf(users[2].address));

    // user tries to burn
    // await expect().to.revertedWith("");
  });
  /*
  it('Withdraw', async () => {
    const { users, pool, dai, aDai, configurator } = testEnv;
